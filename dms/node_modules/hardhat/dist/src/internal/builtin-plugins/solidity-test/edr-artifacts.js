...
export async function getEdrArtifacts(artifactManager) {
    ...
    const buildInfoIds = Array.from(new Set(artifacts.map((artifact) => artifact.buildInfoId)));
    const solcVersionsArray = buildInfoIds
        .map((buildInfoId) => {
        assertHardhatInvariant(buildInfoId !== undefined, "buildInfoId should not be undefined");
    ...
    return artifacts.map((artifact) => {
        assertHardhatInvariant(artifact.buildInfoId !== undefined, "solcVersion should not be undefined");
        const solcVersion = solcVersions.get(artifact.buildInfoId);
        assertHardhatInvariant(solcVersion !== undefined, "solcVersion should not be undefined");
        const id = {
